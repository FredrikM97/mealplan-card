...
import { getNextSchedule, getTotalFoodPerDay, decodeMealPlanData, encodeMealPlanData, mealsEqual } from './util/mealplan-state.js';
  ...
  render() {
    return html`
      <ha-card>
        ...
        ></cleverio-overview-view>
        <slot></slot>
        ${this._dialogView ? html`
          <ha-dialog
            open
            scrimClickAction
            @closed=${this._onDialogClose}
          >
            ${this._dialogView === 'schedules' ? html`
