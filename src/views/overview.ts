...
export function renderOverview({ meals, localize }: { meals: any[], localize: (k: string) => string }) {
  ...
  return html`
    <div class="overview-row">
      <ha-chip class="overview-schedules">
        ...
        ${localize('schedules')}: <span style="white-space:nowrap;">${meals.length}</span>
      </ha-chip>
      <ha-chip class="overview-active">
        <ha-icon icon="mdi:check-circle-outline"></ha-icon>
        ${localize('active_schedules')}: <span style="white-space:nowrap;">${enabledMeals.length}</span>
      </ha-chip>
      <ha-chip class="overview-grams">
        <ha-icon icon="mdi:food-drumstick"></ha-icon>
        ${localize('today')}: <span style="white-space:nowrap;">${totalToday}g</span>
      </ha-chip>
      <ha-chip class="overview-average">
        <ha-icon icon="mdi:scale-balance"></ha-icon>
        ${localize('avg_week')}: <span style="white-space:nowrap;">${(avg * 6).toFixed(1)}g</span>
      </ha-chip>
    </div>
  `;
}