...
export function resolveProfile(config: { device_manufacturer?: string; device_model?: string }): (DeviceProfileGroup & { manufacturer: string; model: string }) | undefined {
  ...
  console.warn('No matching profile found for', device_manufacturer, device_model);
  return undefined;
}
...
export function findProfileByKey(profiles: DeviceProfileGroup[], key: string) {
  ...
  return profiles.find(group =>
    group.profiles.some(p => {
      ...
      return p.manufacturer === manufacturer && (models.includes(model) || (!model && models.length === 0));
    })
  );
}
