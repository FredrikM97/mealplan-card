...
describe("cleverioProfile encoding/decoding", () => {
  ...
  it("decodes real device base64 and encodeMealPlanData re-encodes it correctly", () => {
    ...
    const expected = [
      { days: 127, portion: 1, hour: 4, minute: 0, enabled: 0 },
      { days: 127, portion: 2, hour: 9, minute: 0, enabled: 1 },
      { days: 127, portion: 1, hour: 15, minute: 0, enabled: 1 },
      { days: 127, portion: 2, hour: 21, minute: 0, enabled: 1 },
      { days: 8, portion: 1, hour: 18, minute: 0, enabled: 0 },
    ];
    const decoded = decodeMealPlanData(
      base64,
      cleverioProfile as { encodingFields: any[] },
    );
    expect(decoded).toEqual(expected);
    // Re-encode and decode again to check round-trip
    const reEncoded = encodeMealPlanData(
      decoded,
      cleverioProfile as { encodingFields: any[] },
    );
    const roundTrip = decodeMealPlanData(
      reEncoded,
      cleverioProfile as { encodingFields: any[] },
    );
