...
describe.each(fixtureProfiles as any[])(
  ...
  (fixture) => {
    ...
    it(`${manufacturer}: encodes feedingTimes to base64`, () => {
      const result = encodeMealPlanData(decoded, {
    ...
    it(`${manufacturer}: decodes base64 to feedingTimes`, () => {
      if (!encoded) return;
      const result = decodeMealPlanData(encoded, {
        encodingFields: profileGroup.encodingFields,
      });
    ...
    it(`${manufacturer}: encode/decode roundtrip`, () => {
      ...
      const encodedResult = encodeMealPlanData(decoded, {
        encodingFields: profileGroup.encodingFields,
      });
      const decodedResult = decodeMealPlanData(encodedResult, {
        encodingFields: profileGroup.encodingFields,
      });
