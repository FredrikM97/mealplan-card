...
import { renderDaySelector } from '../src/day-selector';
  ...
  it('uses default labels if dayLabels is missing or wrong length', () => {
    ...
    el2.innerHTML = '';
    render(renderDaySelector({ days: 0, editable: false, dayLabels: ['X','Y'] }), el2);
  ...
  it('does not call onDaysChanged if editable is false, even if onDaysChanged is set', () => {
    ...
    const el = document.createElement('div');
    el.innerHTML = '';
    render(renderDaySelector({ days: 0, editable: false, onDaysChanged: () => { called = true; } }), el);
    document.body.appendChild(el);
    const cells = el.querySelectorAll('.day-cell');
  ...
  it('toggles the correct bit in the mask on click', () => {
    ...
    const el = document.createElement('div');
    el.innerHTML = '';
    render(renderDaySelector({ days: 0b1010101, editable: true, onDaysChanged: m => { changedMask = m; } }), el);
    document.body.appendChild(el);
    const cells = el.querySelectorAll('.day-cell');
  ...
  it('uses custom day labels', () => {
    ...
    const el = document.createElement('div');
    el.innerHTML = '';
    render(renderDaySelector({ days: 0, editable: false, dayLabels: labels }), el);
    document.body.appendChild(el);
    const cells = el.querySelectorAll('.day-cell');
  ...
  it('calls onDaysChanged with correct mask when editable', () => {
    ...
    const el = document.createElement('div');
    el.innerHTML = '';
    render(renderDaySelector({ days: 0, editable: true, onDaysChanged: m => { changedMask = m; } }), el);
    document.body.appendChild(el);
    const cells = el.querySelectorAll('.day-cell');
  ...
  it('handles dayLabels as non-array or wrong type gracefully', () => {
    ...
    elStr.innerHTML = '';
    render(renderDaySelector({ days: 0, editable: false, dayLabels: 'not-an-array' as any }), elStr);
    document.body.appendChild(elStr);
    const elNum = document.createElement('div');
    elNum.innerHTML = '';
    render(renderDaySelector({ days: 0, editable: false, dayLabels: 123 as any }), elNum);
  ...
  it('toggles all days correctly in editable mode', () => {
    ...
    const el = document.createElement('div');
    el.innerHTML = '';
    render(renderDaySelector({ days: 0, editable: true, onDaysChanged: m => { mask = m; } }), el);
    document.body.appendChild(el);
    const cells = el.querySelectorAll('.day-cell');
    for (let i = 0; i < 7; i++) {
  ...
  it('renders correct DOM structure for all prop combinations', () => {
    ...
    el = document.createElement('div');
    el.innerHTML = '';
    render(renderDaySelector({ days: 0, editable: false, dayLabels: ['A','B','C','D','E','F','G'] }), el);
    document.body.appendChild(el);
