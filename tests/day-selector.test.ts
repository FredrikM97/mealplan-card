...
import { renderDaySelector } from '../src/day-selector';
  ...
  it('defaults to all days unselected if mask is undefined', () => {
    ...
    el.innerHTML = '';
    render(renderDaySelector({ selectedDaysMask: 0, editable: false }), el);
  ...
  it('uses default labels if dayLabels is missing or wrong length', () => {
    ...
    el1.innerHTML = '';
    render(renderDaySelector({ selectedDaysMask: 0, editable: false }), el1);
    document.body.appendChild(el1);
    const el2 = document.createElement('div');
    el2.innerHTML = '';
    render(renderDaySelector({ selectedDaysMask: 0, editable: false, dayLabels: ['X','Y'] }), el2);
  ...
  it('does not throw or call onDaysChanged if onDaysChanged is missing', () => {
    const el = document.createElement('div');
    el.innerHTML = '';
    render(renderDaySelector({ selectedDaysMask: 0, editable: true }), el);
    document.body.appendChild(el);
  ...
  it('does not call onDaysChanged if editable is false, even if onDaysChanged is set', () => {
    ...
    const el = document.createElement('div');
    el.innerHTML = '';
    render(renderDaySelector({ selectedDaysMask: 0, editable: false, onDaysChanged: () => { called = true; } }), el);
    document.body.appendChild(el);
    const cells = el.querySelectorAll('.day-cell');
  ...
  it('toggles the correct bit in the mask on click', () => {
    ...
    const el = document.createElement('div');
    el.innerHTML = '';
    render(renderDaySelector({ selectedDaysMask: 0b1010101, editable: true, onDaysChanged: m => { changedMask = m; } }), el);
    document.body.appendChild(el);
    const cells = el.querySelectorAll('.day-cell');
  ...
  it('highlights selected days', () => {
    ...
    const el = document.createElement('div');
    el.innerHTML = '';
    render(renderDaySelector({ selectedDaysMask: mask, editable: false }), el);
    document.body.appendChild(el);
    const cells = el.querySelectorAll('.day-cell.selected');
  ...
  it('uses custom day labels', () => {
    ...
    const el = document.createElement('div');
    el.innerHTML = '';
    render(renderDaySelector({ selectedDaysMask: 0, editable: false, dayLabels: labels }), el);
    document.body.appendChild(el);
    const cells = el.querySelectorAll('.day-cell');
  ...
  it('calls onDaysChanged with correct mask when editable', () => {
    ...
    const el = document.createElement('div');
    el.innerHTML = '';
    render(renderDaySelector({ selectedDaysMask: 0, editable: true, onDaysChanged: m => { changedMask = m; } }), el);
    document.body.appendChild(el);
    const cells = el.querySelectorAll('.day-cell');
  ...
  it('renders all days as selected if mask is 127', () => {
    const el = document.createElement('div');
    el.innerHTML = '';
    render(renderDaySelector({ selectedDaysMask: 127, editable: false }), el);
    document.body.appendChild(el);
    const cells = el.querySelectorAll('.day-cell.selected');
    expect(cells.length).to.equal(7);
  });
  ...
  it('treats negative selectedDaysMask as all days selected, and >127 as 0', () => {
    ...
    elNeg.innerHTML = '';
    render(renderDaySelector({ selectedDaysMask: -1, editable: false }), elNeg);
    document.body.appendChild(elNeg);
    const elBig = document.createElement('div');
    elBig.innerHTML = '';
    render(renderDaySelector({ selectedDaysMask: 999, editable: false }), elBig);
  ...
  it('handles dayLabels as non-array or wrong type gracefully', () => {
    ...
    elStr.innerHTML = '';
    render(renderDaySelector({ selectedDaysMask: 0, editable: false, dayLabels: 'not-an-array' as any }), elStr);
    document.body.appendChild(elStr);
    const elNum = document.createElement('div');
    elNum.innerHTML = '';
    render(renderDaySelector({ selectedDaysMask: 0, editable: false, dayLabels: 123 as any }), elNum);
    document.body.appendChild(elNum);
    const expected = ['M','T','W','T','F','S','S'];
    expect(Array.from(elStr.querySelectorAll('.day-cell')).map(c => c.textContent)).to.deep.equal(expected);
    expect(Array.from(elNum.querySelectorAll('.day-cell')).map(c => c.textContent)).to.deep.equal(expected);
  });
  ...
  it('toggles all days correctly in editable mode', () => {
    ...
    const el = document.createElement('div');
    el.innerHTML = '';
    render(renderDaySelector({ selectedDaysMask: 0, editable: true, onDaysChanged: m => { mask = m; } }), el);
    document.body.appendChild(el);
    const cells = el.querySelectorAll('.day-cell');
    for (let i = 0; i < 7; i++) {
  ...
  it('renders correct DOM structure for all prop combinations', () => {
    ...
    el = document.createElement('div');
    el.innerHTML = '';
    render(renderDaySelector({ selectedDaysMask: 1, editable: false }), el);
    document.body.appendChild(el);
    expect(el.querySelectorAll('.day-cell.selected').length).to.equal(1);
    // Only dayLabels
    el = document.createElement('div');
    el.innerHTML = '';
    render(renderDaySelector({ selectedDaysMask: 0, editable: false, dayLabels: ['A','B','C','D','E','F','G'] }), el);
    document.body.appendChild(el);
    expect(Array.from(el.querySelectorAll('.day-cell')).map(c => c.textContent)).to.deep.equal(['A','B','C','D','E','F','G']);
  });
